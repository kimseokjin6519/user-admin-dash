   ...
   return (
      <div className="fixed w-[250px] top-0 left-0 bg-white">
         <div className="flex w-[250px] bg-blue-800 mb-4">
            <div
               className={`absolute top-0 left-0 w-0 h-0 border-l-[15px] border-b-[15px] border-l-transparent border-b-white transition-opacity duration-500 ease-out ${ allItemsVisible ? 'opacity-0' : 'opacity-100'}`}></div>
         ...
         {items.map((item, index) => (
            <div key={index} className={`ml-14 mb-2 text-sm transition-transform duration-500 ${visibleItems.includes(index) ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-5'}`} style={{ fontFamily: 'Google Sans' }}>
               {item}
            </div>
         ))}
      </div>
